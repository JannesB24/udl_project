...
def create_comprehensive_plots(results):
    ...
    colors = {'Original ResNet': 'red', 
              'L2 Regularized ResNet': 'blue', 
              'Ensemble ResNet': 'green'}
    linestyles = {'Original ResNet': '-', 
                  'L2 Regularized ResNet': '--', 
                  'Ensemble ResNet': '-.'}
    
    # Main comparison plot - 2x2 subplots
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle('UDL Project - Comprehensive Regularization Comparison', fontsize=16, fontweight='bold')
    
    # Plot 1: Training Loss
    axes[0, 0].set_title('Training Loss Comparison', fontweight='bold', fontsize=14)
    for name, data in results.items():
        axes[0, 0].plot(data['train_losses'], label=name, linewidth=2.5, 
    ...
    for name, data in results.items():
        axes[0, 1].plot(data['val_losses'], label=name, linewidth=2.5,
                       color=colors[name], linestyle=linestyles[name])
    ...
    for name, data in results.items():
        axes[1, 0].plot(data['train_accs'], label=name, linewidth=2.5,
                       color=colors[name], linestyle=linestyles[name])
    ...
    for name, data in results.items():
        axes[1, 1].plot(data['val_accs'], label=name, linewidth=2.5,
                       color=colors[name], linestyle=linestyles[name])
    ...
    for name, data in results.items():
        ...
        axes[0].plot(overfitting_gap, label=name, linewidth=3,
                    color=colors[name], linestyle=linestyles[name])
    ...
    bars = axes[1].bar(range(len(names)), gaps, 
                      color=[colors[name] for name in names], alpha=0.7, edgecolor='black')
    ...
    bars = axes[2].bar(range(len(names)), val_accs, 
                      color=[colors[name] for name in names], alpha=0.7, edgecolor='black')
    ...
    for name, data in results.items():
        ...
        plt.plot(data['val_losses'], color=colors[name], linestyle='--', linewidth=2, label=f'{name}')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    ...
    original_gap = results['Original ResNet']['train_accs'][-1] - results['Original ResNet']['val_accs'][-1]
    gap_reductions = []
    method_names = []
    ...
    bars = plt.bar(method_names, gap_reductions, 
    ...
    for name, data in results.items():
        ...
        if name == 'Original ResNet':
            ...
            val_improvement = ((val_acc - results['Original ResNet']['val_accs'][-1]) / 
                             results['Original ResNet']['val_accs'][-1] * 100)
    ...
    best_method = min(results.items(), key=lambda x: x[1]['train_accs'][-1] - x[1]['val_accs'][-1])[0]
